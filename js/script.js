...
function getTopRatedMovies() {
  fetch(
    `https://api.themoviedb.org/3/movie/top_rated?api_key=b9b6aeb3b55201c50bafd1254bcacc66&language=en-US&page=1`
  )
    .then((response) => response.json())
    .then((data) => {
      const results = data.results;
      showTopRatedMovies(results);
    });
}

function getPopularMovies() {
  fetch(`
      https://api.themoviedb.org/3/movie/popular?api_key=b9b6aeb3b55201c50bafd1254bcacc66&language=en-US&page=1`)
    .then((response) => response.json())
    .then((data) => {
      const results = data.results;
...
const showTopRatedMovies = (movies) => {
  let topRatedCards = "";
  const topRated = document.querySelector(".top-rated-card");

  for (let index = 0; index < 10; index++) {
    const data = movies[index];
    topRatedCards += showCards(data);
  }
  topRated.innerHTML = topRatedCards;
};
